...
#         self.d1_conv = [convbnrelu(layer, kernel_size=d_kernel_size, stride=d_stride, padding=d_padding, bn_zero_init=False, bn_statistics_group_size=bn_stats_grp_sz, relu=False, name=self.name+'_disc1_conv'+str(i)) for i,layer in enumerate(d_neurons)]
        ...
        self.d1_conv = [conv(layer,d_kernel_size, stride=d_stride, padding=d_padding, transpose=False, bias= True, weights=[lbann.Weights(initializer=self.inits['conv']),lbann.Weights(initializer=lbann.ConstantInitializer(value=0.0))], name=self.name+'_disc1_conv'+str(i)) for i,layer in enumerate(d_neurons)]
...
#         self.d2_conv = [convbnrelu(layer, d_kernel_size, d_stride, d_padding, False, bn_stats_grp_sz, False,name=self.name+'_disc2_conv'+str(i)) for i,layer in enumerate(d_neurons)] 
        ...
        self.d2_conv = [conv(layer,d_kernel_size, stride=d_stride, padding=d_padding, transpose=False, bias=True, weights=[lbann.Weights(initializer=self.inits['conv']),lbann.Weights(initializer=lbann.ConstantInitializer(value=0.0))], name=self.name+'_disc2_conv'+str(i)) for i,layer in enumerate(d_neurons)]
...
#             else : x = lbann.LeakyRelu(lyr(x), negative_slope=0.2)
            #### without convbrlu
            if count==0: x = lbann.LeakyRelu(lbann.BatchNormalization(lyr(img),decay=0.9,scale_init=1.0,epsilon=1e-5),negative_slope=0.2)
            else: x = lbann.LeakyRelu(lbann.BatchNormalization(lyr(x),decay=0.9,scale_init=1.0,epsilon=1e-5),negative_slope=0.2)
